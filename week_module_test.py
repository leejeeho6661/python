...
class adminClass:
    ...
    def itemAll(self):
        ...
        if len(rows) == 0:
            ...
            print("{:<3}\t{:<10}\t\t{:^10}\t{:<10}\t{}".format('ID','상품명','가격','수량','등록 날짜'))
            for row in rows:
                print("{:<3}\t{:<10}\t\t{:>10,}원\t{:<4}\t{}".format(row['id'],row['product_name'],row['product_price'],row['product_qty'],row['created_at']))
...
class normalClass:
    ...
    def searchItem(self):
        ...
        sql = "select product_name,product_price,product_qty from item where product_name like '%"+item+"%'"
        try:
            cursor.execute(sql)
            rows = cursor.fetchall()
        ...
        if len(row) != 0:
            try:
                ...
                try:
                    ...
                    print(item)
                    print(type(count))
                    print(str(int(row['product_price'])*int(count)))
    ...
    def itemList(self):
        sql = 'select product_name,product_price,product_qty from item'
        cursor.execute(sql)
        rows = cursor.fetchall()
        print('{:<10}\t{:<10}\t{:<5}'.format('상품명','가격','수량'))
        for row in rows:
